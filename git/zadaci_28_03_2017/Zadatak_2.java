...
public class Zadatak_2 {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * insert the statement package chapteri; as the first line for each
		 * Java source file under the directory chapteri. Suppose chapter1,
		 * chapter2, . . . , chapter34 are under the root directory
		 * srcRootDirectory. The root directory and chapteri directory may
		 * contain other folders and files. Use the following command to run the
		 * program: java Exercise12_18 srcRootDirectory
		 */
	...
	public static void filterJavaFiles(ArrayList<File> list) {
		for (int i = 0; i < list.size(); i++) {
			...
			boolean isjavaCode = str.substring(str.lastIndexOf('.')).equals(
					".java");
			if (!isjavaCode)
				list.remove(i);
		}
